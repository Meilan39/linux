...
atomic64_t pcache_pks_scope_begin_count = ATOMIC64_INIT(0);
EXPORT_SYMBOL_GPL(pcache_pks_scope_begin_count);

atomic64_t pcache_pks_scope_end_count = ATOMIC64_INIT(0);
EXPORT_SYMBOL_GPL(pcache_pks_scope_end_count);
...
static int pcache_pks_status_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "scope_begin_count: %lld\n",
		   (long long)atomic64_read(&pcache_pks_scope_begin_count));
	seq_printf(m, "scope_end_count:   %lld\n",
		   (long long)atomic64_read(&pcache_pks_scope_end_count));
	return 0;
}
