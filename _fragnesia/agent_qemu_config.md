can you take a look at the updated information in [README.md], [run_qemu.sh], [fragnesia.c], and [fragnesia_pks.c]. I want you to walk me through how I am going to chain these together to make a valid test, including the correct clean-up for the exploit (very important), and additional measures for Ubuntu that the README specifies. I would like to run a pks unprotected [fragnesia.c] and a protected [fragnesia_pks.c], and look at the usr/bin/su to determine if the exploit was sucessfully blocked. So three things I would like you to do in particular. 

1. Ensure that [run_qemu.sh] is set up correctly for instantiating qemu with fragnesia compatibility, and all dependencies.
2. Make walkthrough of the steps.
3. Implment a run.sh with a parameter of either executable, which runs the excutable with the correct configuration and safegaurds as well as the clean-up.
...
//        file_size = use_existing_target("/tmp/test");
    ...
    /* set /usr/bin/su to key 1 and disallow write */
    pks_protection();
    /* end */
...
EXPLOIT_DIR="$HOME/exploit_src" # Drop your compiled binaries here
...
# Create the exploit folder on the host if it doesn't exist
if [ ! -d "$EXPLOIT_DIR" ]; then
    mkdir -p "$EXPLOIT_DIR"
    echo "Created $EXPLOIT_DIR."
    echo "Place your compiled Fragnesia PoC in this folder before running."
fi
...
# Clean previous build to prevent stale libraries from piling up
rm -rf "$SANDBOX_DIR"
...
# 2. Function to safely copy dynamic binaries and their shared libraries
install_dynamic_bin() {
    local BIN_PATH=$(which $1)
    if [ -z "$BIN_PATH" ]; then
        echo "Error: Command '$1' not found on host OS."
        exit 1
    fi
    
    echo "Injecting $1 and its shared libraries..."
    cp "$BIN_PATH" "$SANDBOX_DIR/bin/"
    
    # Use ldd to find dependencies, format the output, and copy them
    ldd "$BIN_PATH" | awk '
        /=>/ { if ($3 ~ /^\//) print $3 }
        /^\t\// { print $1 }
    ' | while read -r lib; do
        # Create the specific lib directory structure inside the sandbox
        mkdir -p "$SANDBOX_DIR$(dirname "$lib")"
        # Copy the actual library file
        cp -L "$lib" "$SANDBOX_DIR$lib"
    done
}

# Inject the full iproute2 'ip' tool for XFRM/IPsec support
install_dynamic_bin "ip"

# 3. Inject user exploits
echo "Copying files from $EXPLOIT_DIR..."
cp -r "$EXPLOIT_DIR/"* "$SANDBOX_DIR/exploit/" 2>/dev/null || true
...
# Bring up the loopback interface (Required for local TCP exploits)
/bin/ip link set lo up

echo "================================================="
echo " PKS Fragnesia Sandbox (Network & IPsec Ready) "
echo "================================================="
echo "Your exploits are located in /exploit/"
echo "Type 'exit' to panic the kernel or Ctrl+A, X to quit."
echo "================================================="
