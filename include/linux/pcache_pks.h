...
extern atomic64_t pcache_pks_scope_count;
static inline void pcache_pks_count_scope(bool cond)
{
	if (cond)
		atomic64_inc(&pcache_pks_scope_count);
}
#else
static inline void pcache_pks_count_scope(bool cond) {}
#endif
...
static inline void pcache_pks_scope_clamp(struct pcache_pks_scope *scope,
					  bool active)
{
	scope->active = active;
	if (active)
